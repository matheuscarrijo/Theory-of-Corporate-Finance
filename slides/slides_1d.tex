...
\begin{block}{At $t = 0$}
\begin{equation*}
0 \leq x^i_0 := \omega^i_0 + q_i - q \cdot z^i - k^i
\end{equation*}
\end{block}

\begin{block}{At $t = 1$ in state $s$}
\begin{equation*}
0 \leq x^i_s := V_s(f^i(k^i, e^i), y^{-i}) \cdot z^i
\end{equation*}
\end{block}
\bigskip
...
\begin{block}{At $t = 0$}
\begin{equation*}
0 \leq x^i_0 := \omega^i_0 - q \cdot z^i
\end{equation*}
\end{block}

\begin{block}{At $t = 1$ in state $s$}
\begin{equation*}
0 \leq x^i_s := V_s(0, y^{-i}) \cdot z^i
\end{equation*}
\end{block}
\bigskip
...
\begin{equation*}
\widetilde{e}^i(k^i, z^i, y^{-i}) := \argmax \bigl\{ u^i_1(x^i_1, e^i) \colon e^i \in E^i,\ x^i_1 = V(f^i(k^i, e^i), y^{-i}) z^i \bigr\}
\end{equation*}
\bigskip
...
Assume:
\begin{itemize}
\item[(a)] $J = I \cup \{j_0\}$ (only bond and equities)
...
\begin{definitionns}
$(\bm{x}, \bm{e}) = (x^i, e^i)_{i \in I}$ is \alert{constrained feasible} if there exist $(\bm{k}, \bm{z})$ such that:
\end{definitionns}

\begin{itemize}
\item[(i)] production: $y^i = f^i(k^i, e^i)$
\item[(ii)] optimal effort: $e^i \in \widetilde{e}^i(k^i, z^i, y^{-i})$
\item[(iii)] risk-sharing: $x^i_s = V_s(y) \cdot z^i$ for all $s \in S$
\item[(iv)] date-$0$ market clearing: $\sum_i (x^i_0 + k^i) = \sum_i \omega^i_0$
...
\begin{definitionns}
A constrained feasible allocation $(\bm{x}, \bm{e})$ is \alert{constrained Pareto optimal} (CPO) if there is no other constrained feasible $(\widehat{\bm{x}}, \widehat{\bm{e}})$ with
\begin{equation*}
U^i(\widehat{x}^i, \widehat{e}^i) \geq U^i(x^i, e^i), \quad \text{for all $i \in I$}
\end{equation*}
...
Let $\bigl\{ (\overline{x}^i, \overline{e}^i, \overline{k}^i, \overline{z}^i)_i, (\overline{y}^j)_j, \overline{q} \bigr\}$ be an RCPP equilibrium. Suppose:
\begin{itemize}
...
\end{itemize}
Then $(\overline{x}^i, \overline{e}^i)_{i \in I}$ is constrained Pareto optimal.
\end{theoremns}
\end{frame}
...
Suppose for contradiction that $(\bm{x}, \bm{e})$ is constrained feasible and Pareto-dominates $(\overline{\bm{x}}, \overline{\bm{e}})$
\bigskip
...
\item Compute the implied date-$0$ consumption $\widehat{x}^i_0$
\item Use RCPP-optimality to conclude $x^i_0 \geq \widehat{x}^i_0$ for all $i$, with strict inequality for at least one
...
\begin{equation*}
\widehat{x}^i_1 = \sum_{j \in J^i} V_j(f^i(k^i, e^i)) z^i_j + m^i_1 = x^i_1
\end{equation*}
\bigskip

Since $u^i_1$ depends only on $(x^i_1, e^i)$, the argmax sets coincide
\end{frame}
...
\begin{equation*}
\widehat{x}^i_0 = \omega^i_0 + \overline{\pi} \cdot f^i(k^i, \widehat{e}^i) - \overline{\pi} \cdot \widehat{x}^i_1 - k^i
\end{equation*}
\bigskip

Choosing $\widehat{e}^i = e^i$ (in the common argmax by Lemma) and using $\widehat{x}^i_1 = x^i_1$:
\begin{equation*}
\widehat{x}^i_0 = \omega^i_0 + \overline{\pi} \cdot y^i - \overline{\pi} \cdot x^i_1 - k^i
\end{equation*}
\end{frame}

%--------------------------------------------------------------------
\begin{frame}\frametitle{Proof: individual comparison}
The alternative $(\widehat{k}^i, \widehat{z}^i, \widehat{x}^i_0, x^i_1, e^i)$ is in the RCPP budget set
\bigskip
...
\begin{equation*}
x^i_0 \geq \widehat{x}^i_0, \quad \text{for all $i \in I$}
\end{equation*}
...
Summing over $i$:
\begin{equation*}
\sum_{i \in I} x^i_0 > \sum_{i \in I} \omega^i_0 + \overline{\pi} \cdot \sum_{i \in I} y^i - \overline{\pi} \cdot \sum_{i \in I} x^i_1 - \sum_{i \in I} k^i
\end{equation*}
\bigskip

By date-$0$ market clearing: $\sum_i (x^i_0 + k^i) = \sum_i \omega^i_0$
\bigskip

By portfolio market clearing: $\sum_i x^i_1 = \sum_i y^i$
\bigskip
