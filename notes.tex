...
\begin{enumerate}
  ...
  \end{equation*}
  \item[\textup{(ii)}] it is implementable with the existing asset structure: there exists $\beta \geq 0$ with $\beta \leq \inf_{s\in S} f_s(k)$ such that, for every $i$, one can find $(\theta^i, \alpha^i) \in \R_+ \times \R_+$ satisfying
  \begin{equation*}
...
The condition $\beta \leq \inf_s f_s(k)$ is the no-default constraint: the firm's chosen $(k, \beta)$ must lie in $\mathcal{D}$, so that the dividend $f_s(k) - \beta$ is non-negative in every state and the equity claim is well-defined.
...
\subsection{Weak Pareto efficiency}

The familiar notion of Pareto efficiency rules out admissible allocations that weakly dominate the equilibrium with at least one strict improvement.
For our welfare result it is convenient to work with a slightly weaker concept: the equilibrium cannot be \emph{strictly} dominated, in the sense that no admissible allocation makes \emph{every} agent strictly better off.

\begin{definition}[Weak Pareto efficiency]\label{def:weak-pareto-BGR}
An admissible allocation $(c^i)_{i \in I}$ is \emph{weakly constrained Pareto efficient} if there does not exist another admissible allocation $(\widetilde{c}^i)_{i \in I}$ such that
\begin{equation*}
  U^i(\widetilde{c}^i) > U^i(c^i), \quad \text{for all $i \in I$.}
\end{equation*}
\end{definition}

Strong (constrained) Pareto efficiency, in which one rules out admissible allocations satisfying $U^i(\widetilde{c}^i) \geq U^i(c^i)$ for all $i$ with at least one strict inequality, is a stronger requirement: any strongly Pareto efficient allocation is automatically weakly Pareto efficient.

\begin{remark}[Weak vs.\ strong Pareto]\label{rem:weak-vs-strong}
Under continuity and strict monotonicity of the utility functions $U^i$, the two notions coincide: a weakly constrained Pareto efficient allocation is also strongly constrained Pareto efficient.
The argument is standard: from a weak Pareto improvement satisfying $U^i(\widetilde{c}^i) \geq U^i(c^i)$ for all $i$ with strict inequality at $i^\ast$, one can transfer a small amount of date-$0$ consumption from $i^\ast$ to each of the other agents, producing a strict improvement for everyone.
See \citet{Mas-Colell-Whinston-Green-1995}, Proposition~16.D.2, or \cite{BisinGottardiRuta2015} for a detailed treatment in the present setting.
\end{remark}

\subsection{The welfare theorem}

\begin{theorem}[Weak welfare theorem]\label{thm:welfare-BGR}
Under Assumption~\ref{ass:C}, every competitive-equilibrium allocation is weakly constrained Pareto efficient.
\end{theorem}

\begin{proof}
Suppose, by contradiction, that the equilibrium allocation $(c^i)_{i \in I}$ is weakly Pareto-dominated by an admissible allocation $(\widetilde{c}^i)_{i \in I}$, i.e.,
\begin{equation}\label{eq:weak-domination}
  U^i(\widetilde{c}^i) > U^i(c^i), \quad \text{for all $i \in I$.}
\end{equation}
Let $(\widetilde{k}, \widetilde{\beta}) \in \mathcal{D}$ and $(\widetilde{\theta}^i, \widetilde{\alpha}^i)_{i \in I} \in (\R_+ \times \R_+)^I$ be a supporting investment, debt, and portfolio profile from Definition~\ref{def:admissible-alloc}.

\paragraph{Step 1: A wealth gap is negative for every agent.}
For each $i \in I$, define
\begin{equation}\label{eq:wealth-gap}
  G^i \coloneqq \omega^i_0 + V(\widetilde{k}, \widetilde{\beta}) \xi^i - \widetilde{c}^i_0 - q \widetilde{\theta}^i - \widetilde{E}(\widetilde{k}, \widetilde{\beta}) \widetilde{\alpha}^i.
\end{equation}
\textbf{Claim:} $G^i < 0$ for every $i \in I$.

Suppose, for the sake of contradiction, that $G^{i^\ast} \geq 0$ for some $i^\ast$.
The portfolio $(\widetilde{\theta}^{i^\ast}, \widetilde{\alpha}^{i^\ast}) \in \R_+ \times \R_+$ at firm choice $(\widetilde{k}, \widetilde{\beta})$ and prices $(q, \widetilde{E}(\widetilde{k}, \widetilde{\beta}))$ implements the consumption $\widetilde{c}^{i^\ast}$:
the date-$1$ identity holds with equality by admissibility, and the date-$0$ budget constraint holds because $G^{i^\ast} \geq 0$.
Therefore $\widetilde{c}^{i^\ast}$ belongs to the budget set $B^{i^\ast}\bigl(q, \widetilde{E}(\widetilde{k}, \widetilde{\beta}), \widetilde{k}, \widetilde{\beta}\bigr)$ used in the strong-unanimity statement.
By Theorem~\ref{thm:strong-unanimity-BGR},
\begin{equation*}
  U^{i^\ast}(c^{i^\ast}) \geq U^{i^\ast}(\widetilde{c}^{i^\ast}),
\end{equation*}
contradicting \eqref{eq:weak-domination} at $i = i^\ast$.
The claim is established and, summing over $i$,
\begin{equation}\label{eq:sum-G-neg}
  \sum_{i \in I} G^i < 0.
\end{equation}

\paragraph{Step 2: Aggregating the wealth gap.}
Sum \eqref{eq:wealth-gap} over $i$, using $\sum_i \xi^i = 1$ and $V(\widetilde{k}, \widetilde{\beta}) = \widetilde{E}(\widetilde{k}, \widetilde{\beta}) + q\widetilde{\beta} - \widetilde{k}$:
\begin{equation*}
  \sum_{i \in I} G^i
  = \biggl[ \sum_{i \in I} \omega^i_0 - \sum_{i \in I} \widetilde{c}^i_0 - \widetilde{k} \biggr]
  + \widetilde{E}(\widetilde{k}, \widetilde{\beta}) (1 - A)
  + q (\widetilde{\beta} - T),
\end{equation*}
where $T \coloneqq \sum_{i \in I} \widetilde{\theta}^i$ and $A \coloneqq \sum_{i \in I} \widetilde{\alpha}^i$.
By the date-$0$ feasibility part of admissibility, the bracketed term is non-negative, so
\begin{equation}\label{eq:sum-G-bound}
  \sum_{i \in I} G^i \geq \widetilde{E}(\widetilde{k}, \widetilde{\beta}) (1 - A) + q(\widetilde{\beta} - T).
\end{equation}

\paragraph{Step 3: The right-hand side of \eqref{eq:sum-G-bound} is non-negative.}
We split the argument into two cases according to the sign of $1 - A$.

\textit{Case $A \leq 1$.}
By Proposition~\ref{prop:max-pricing}, there exists an investor $j^\ast \in I^b$, i.e., with $\PV^{j^\ast}[\1_S] = q$.
Set
\begin{equation*}
  \Pi^\ast \coloneqq \PV^{j^\ast}[f_1(\widetilde{k}) - \widetilde{\beta} \1_S],
\end{equation*}
which by the Makowski criterion satisfies $\Pi^\ast \leq \widetilde{E}(\widetilde{k}, \widetilde{\beta})$.
Multiply the date-$1$ feasibility inequality $T + (f_s(\widetilde{k}) - \widetilde{\beta}) A \leq f_s(\widetilde{k})$ (for each $s$) by $\MRS^{j^\ast}_s$ and sum over $s \in S$:
\begin{equation*}
...
\end{equation*}
where the second inequality uses $A \leq 1$ and $\Pi^\ast \leq \widetilde{E}(\widetilde{k}, \widetilde{\beta})$.
Rearranging,
\begin{equation}\label{eq:case-A-le-1}
  \widetilde{E}(\widetilde{k}, \widetilde{\beta})(1 - A) + q(\widetilde{\beta} - T) \geq 0.
\end{equation}

\textit{Case $A > 1$.}
We first show that $T \leq \widetilde{\beta}$.
The date-$1$ feasibility inequality at $s = \underline{s} \coloneqq \arg\min_{s'} f_{s'}(\widetilde{k})$ reads
\begin{equation*}
  T + (f_{\underline{s}}(\widetilde{k}) - \widetilde{\beta}) A \leq f_{\underline{s}}(\widetilde{k}),
\end{equation*}
i.e., $T \leq f_{\underline{s}}(\widetilde{k})(1 - A) + \widetilde{\beta} A$.
The no-default condition $\widetilde{\beta} \leq f_{\underline{s}}(\widetilde{k})$ combined with $1 - A < 0$ yields
$f_{\underline{s}}(\widetilde{k})(1 - A) \leq \widetilde{\beta}(1 - A)$, so
\begin{equation*}
  T \leq \widetilde{\beta}(1 - A) + \widetilde{\beta} A = \widetilde{\beta}.
\end{equation*}
Hence $\widetilde{\beta} - T \geq 0$.

Now choose an investor $\widehat{i}$ achieving the Makowski max, i.e., $\PV^{\widehat{i}}[f_1(\widetilde{k}) - \widetilde{\beta} \1_S] = \widetilde{E}(\widetilde{k}, \widetilde{\beta})$.
By Proposition~\ref{prop:max-pricing}, $\PV^{\widehat{i}}[\1_S] \leq q$.
Multiplying the date-$1$ feasibility inequality by $\MRS^{\widehat{i}}_s$ and summing,
\begin{equation*}
  T \PV^{\widehat{i}}[\1_S] + A \widetilde{E}(\widetilde{k}, \widetilde{\beta})
  \leq \PV^{\widehat{i}}[f_1(\widetilde{k})]
  = \widetilde{E}(\widetilde{k}, \widetilde{\beta}) + \widetilde{\beta} \PV^{\widehat{i}}[\1_S],
\end{equation*}
or equivalently
\begin{equation*}
  \PV^{\widehat{i}}[\1_S](T - \widetilde{\beta}) \leq (1 - A) \widetilde{E}(\widetilde{k}, \widetilde{\beta}).
\end{equation*}
Reading this as $\widetilde{E}(\widetilde{k}, \widetilde{\beta})(A - 1) \leq \PV^{\widehat{i}}[\1_S](\widetilde{\beta} - T)$, and using $\widetilde{\beta} - T \geq 0$ together with $\PV^{\widehat{i}}[\1_S] \leq q$,
\begin{equation*}
  \widetilde{E}(\widetilde{k}, \widetilde{\beta})(A - 1) \leq \PV^{\widehat{i}}[\1_S](\widetilde{\beta} - T) \leq q(\widetilde{\beta} - T),
\end{equation*}
which is once more \eqref{eq:case-A-le-1}.

\paragraph{Conclusion.}
Combining \eqref{eq:sum-G-bound} and \eqref{eq:case-A-le-1}, $\sum_i G^i \geq 0$, contradicting \eqref{eq:sum-G-neg}.
The supposed weakly Pareto-dominating allocation cannot exist.
\end{proof}

The welfare theorem tightens the unanimity statement of Theorem~\ref{thm:strong-unanimity-BGR} from individual shareholder consent to aggregate efficiency.
...
\begin{remark}[Strength of the conclusion]\label{rem:strength-of-conclusion}
By Remark~\ref{rem:weak-vs-strong}, under standard continuity and monotonicity assumptions on $(U^i)_{i \in I}$, weak constrained Pareto efficiency implies strong constrained Pareto efficiency.
The competitive-equilibrium allocation of the Bisin--Gottardi--Ruta economy is therefore Pareto efficient within the constrained class, in the strongest sense available.
\end{remark}
...
The analysis hinges on a single structural property---the convexity of the Makowski value map $\beta' \mapsto V(k, \beta')$---and on a sharp consequence of Danskin's theorem at an interior maximizer.

\subsection{Convexity of the value map}

\begin{lemma}[Convexity of $V(k, \cdot)$]\label{lem:V-convex}
For each fixed $k \geq 0$, the value map
\begin{equation*}
...
\begin{proof}
For each $i \in I$, the function $\beta' \mapsto \PV^i[f_1(k) - \beta' \1_S]$ is affine in $\beta'$:
\begin{equation*}
  \PV^i[f_1(k) - \beta' \1_S] = \PV^i[f_1(k)] - \beta' \PV^i[\1_S].
\end{equation*}
By the Makowski criterion \eqref{eq:makowski},
\begin{equation*}
  \widetilde{E}(k, \beta') = \max_{i \in I} \PV^i[f_1(k) - \beta' \1_S]
\end{equation*}
is the pointwise maximum of finitely many affine functions, hence convex piecewise linear on $\R$.
The map $V(k, \beta')$ is the sum of $\widetilde{E}(k, \cdot)$ and the affine function $\beta' \mapsto q\beta' - k$, hence also convex piecewise linear.
\end{proof}
...
Let
\begin{align*}
  I^e &\coloneqq \bigl\{ i \in I \colon E = \PV^i[f_1(k) - \beta \1_S] \bigr\}, \\
  I^b &\coloneqq \bigl\{ i \in I \colon q = \PV^i[\1_S] \bigr\}.
\end{align*}
An investor $i \in I^e$ is called a (marginal) \emph{equity holder}, and an investor $i \in I^b$ a (marginal) \emph{bond holder}.
\end{definition}
...
By Proposition~\ref{prop:max-pricing}, $E = \max_{i \in I} \PV^i[f_1(k) - \beta \1_S]$ and $q = \max_{i \in I} \PV^i[\1_S]$, so $I^e$ and $I^b$ are both non-empty.

% \subsection{At a value-maximizing equilibrium, equity holders are bond holders}

% The next result shows that, when the firm maximizes its value at an interior point of the no-default region, every equity holder is also a bond holder.
% The argument relies on \emph{Danskin's theorem} \citep{Danskin1966} for the directional derivatives of the maximum of finitely many differentiable functions; for textbook treatments see \citet{Rockafellar1970}, Theorem~23.4--23.5, or \citet{Bertsekas1999}, Proposition~B.25.
...
% \begin{equation*}
%   g_i(\beta') \coloneqq \PV^i[f_1(k)] - \beta' \PV^i[\1_S], \quad \beta' \in \R,
% \end{equation*}
% is affine with constant slope $g_i'(\beta') = -\PV^i[\1_S]$.
% The Makowski equity price coincides with $\widetilde{E}(k, \beta') = \max_{i \in I} g_i(\beta')$, and its active set at $\beta$ is precisely $I^e$ (Definition~\ref{def:marginal-holders}).

% By Danskin's theorem, the one-sided directional derivatives of $\widetilde{E}(k, \cdot)$ at $\beta$ are
% \begin{align}
%   \partial^+ \widetilde{E}(k, \beta)
%   &\coloneqq \lim_{h \downarrow 0} \frac{\widetilde{E}(k, \beta + h) - \widetilde{E}(k, \beta)}{h}
%   = \max_{i \in I^e} g_i'(\beta)
%   = -\min_{i \in I^e} \PV^i[\1_S],
%   \label{eq:Danskin-right-E} \\
%   \partial^- \widetilde{E}(k, \beta)
%   &\coloneqq \lim_{h \downarrow 0} \frac{\widetilde{E}(k, \beta) - \widetilde{E}(k, \beta - h)}{h}
%   = \min_{i \in I^e} g_i'(\beta)
%   = -\max_{i \in I^e} \PV^i[\1_S].
%   \label{eq:Danskin-left-E}
% \end{align}
% Adding the contribution of the linear term $q \beta' - k$,
% \begin{align}
%   \partial^+ V(k, \beta) &= q - \min_{i \in I^e} \PV^i[\1_S],
%   \label{eq:Danskin-right-V} \\
%   \partial^- V(k, \beta) &= q - \max_{i \in I^e} \PV^i[\1_S].
%   \label{eq:Danskin-left-V}
% \end{align}

% By Lemma~\ref{lem:V-convex}, $V(k, \cdot)$ is convex, so $\partial^- V(k, \beta) \leq \partial^+ V(k, \beta)$.
% Because $\beta$ lies in the open interval $\bigl( 0, \inf_{s \in S} f_s(k) \bigr)$ and is a local maximizer of $V(k, \cdot)$, the standard first-order conditions for an interior maximum require
% \begin{equation*}
...
% \end{equation*}
% Substituting \eqref{eq:Danskin-right-V}--\eqref{eq:Danskin-left-V} yields
% \begin{equation*}
%   \min_{i \in I^e} \PV^i[\1_S] = \max_{i \in I^e} \PV^i[\1_S] = q,
% \end{equation*}
% which is to say $\PV^i[\1_S] = q$ for every $i \in I^e$.
% Equivalently, $I^e \subset I^b$.
% \end{proof}

% \subsection{Modigliani--Miller in the Bisin--Gottardi--Ruta framework}

% The next proposition states the irrelevance result.
% The hypothesis is exactly the same as in Proposition~\ref{prop:eh-are-bh}: a value-maximizing equilibrium with strictly slack no-default constraint.
% In particular, no additional assumption on the structure of $I^e$ and $I^b$ is required: convexity of the value map combined with the existence of an interior maximizer is sufficient.
...
% The value map $V(k, \cdot)$ is constant on $\bigl[ 0, \inf_{s \in S} f_s(k) \bigr]$:
% \begin{equation*}
%   V(k, \beta') = V(k, 0) = \widetilde{E}(k, 0) - k, \quad \text{for all $\beta' \in \bigl[ 0, \inf_{s \in S} f_s(k) \bigr]$.}
% \end{equation*}
% \end{proposition}

% \begin{proof}
% By Lemma~\ref{lem:V-convex}, $V(k, \cdot)$ is convex piecewise linear, hence continuous, on $\bigl[ 0, \inf_{s \in S} f_s(k) \bigr]$.
% By hypothesis, $\beta$ lies in the open interval $\bigl( 0, \inf_{s \in S} f_s(k) \bigr)$ and maximizes $V(k, \cdot)$ over $\mathcal{A}$.

% Consider any $\beta'_1, \beta'_2$ with $0 < \beta'_1 < \beta < \beta'_2 < \inf_{s \in S} f_s(k)$.
% Pick $\lambda \in (0, 1)$ such that $\lambda \beta'_1 + (1 - \lambda) \beta'_2 = \beta$.
% By convexity,
% \begin{equation*}
%   V(k, \beta) \leq \lambda V(k, \beta'_1) + (1 - \lambda) V(k, \beta'_2).
% \end{equation*}
% By optimality of $\beta$, $V(k, \beta'_1) \leq V(k, \beta)$ and $V(k, \beta'_2) \leq V(k, \beta)$, so
% \begin{equation*}
%   V(k, \beta) \leq \lambda V(k, \beta'_1) + (1 - \lambda) V(k, \beta'_2) \leq V(k, \beta).
% \end{equation*}
% Equality holds throughout, forcing $V(k, \beta'_1) = V(k, \beta) = V(k, \beta'_2)$.
% Since $\beta'_1, \beta'_2$ were arbitrary, $V(k, \cdot)$ is constant on the open interval $\bigl( 0, \inf_{s \in S} f_s(k) \bigr)$.
% By continuity, the constancy extends to the closed interval $\bigl[ 0, \inf_{s \in S} f_s(k) \bigr]$.
% At $\beta' = 0$, the firm has no debt, so $V(k, 0) = \widetilde{E}(k, 0) - k$.
% \end{proof}

% The combination of Lemma~\ref{lem:V-convex} (convexity of $V(k, \cdot)$) and the elementary fact that a convex function attaining a maximum at an interior point of an interval must be constant on the whole interval (proof of Proposition~\ref{prop:mm-BGR}) yields a sharp dichotomy for the firm's value function on the no-default region.

% \begin{proposition}[Modigliani--Miller dichotomy]
% \label{prop:mm-dichotomy}
% For each $k \geq 0$, exactly one of the following holds.
% \begin{enumerate}
%   \item[\textup{(MM)}] $V(k, \cdot)$ is constant on $\bigl[ 0, \inf_{s \in S} f_s(k) \bigr]$. Every $\beta$ in this interval is value-maximizing, and the firm's capital structure is fully indeterminate.
%   \item[\textup{($\neg$MM)}] $V(k, \cdot)$ is not constant. Every value-maximizing $\beta$ lies in $\bigl\{ 0, \inf_{s \in S} f_s(k) \bigr\}$.
% \end{enumerate}
% \end{proposition}

% \begin{proof}
% By Lemma~\ref{lem:V-convex}, $V(k, \cdot)$ is convex piecewise linear, hence continuous, on $[0, \inf_{s \in S} f_s(k)]$. If a value-maximizing $\beta^\ast$ exists in the open interval $\bigl( 0, \inf_{s \in S} f_s(k) \bigr)$, then by the argument given in the proof of Proposition~\ref{prop:mm-BGR}, $V(k, \cdot)$ is constant on the whole interval (case (MM)). Otherwise every value-maximizing $\beta$ lies in the boundary $\{0, \inf_{s \in S} f_s(k)\}$ (case ($\neg$MM)).
% \end{proof}

% The MM regime admits a clean equivalent description in terms of the marginal-holder sets.

% \begin{remark}[Characterization of (MM) via marginal holders]
% \label{rem:mm-via-IeIb}
% Case (MM) of Proposition~\ref{prop:mm-dichotomy} obtains if and only if $I^e(\beta) \subset I^b$ for every $\beta \in [0, \inf_{s \in S} f_s(k)]$, where
% \begin{equation*}
%   I^e(\beta) \coloneqq \bigl\{ i \in I \colon \widetilde{E}(k, \beta) = \PV^i[f_1(k) - \beta \1_S] \bigr\}.
% \end{equation*}
% The forward direction combines $V(k, \cdot)$ constant with Danskin's theorem: both one-sided derivatives vanish at every $\beta$, so $\min_{i \in I^e(\beta)} \PV^i[\1_S] = \max_{i \in I^e(\beta)} \PV^i[\1_S] = q$, and max-valuation pricing (Proposition~\ref{prop:max-pricing}) forces $\PV^i[\1_S] = q$ for every $i \in I^e(\beta)$. The reverse direction is immediate: if $\PV^i[\1_S] = q$ at every active $i$ and every $\beta$, then by Danskin's theorem both one-sided derivatives of $V(k, \cdot)$ vanish identically.
% \end{remark}

% \begin{remark}[Corner solutions in the ($\neg$MM) regime]
% \label{rem:mm-corners}
% The non-Modigliani--Miller regime splits into two economically distinct sub-cases according to which endpoint the firm selects.
% \begin{itemize}
%   \item \emph{$\beta^\ast = 0$ (all-equity financing).} The right-derivative at $\beta = 0$ is strictly positive, which by Danskin's theorem is equivalent to
%   \begin{equation*}
%     q > \min_{i \in I^e(0)} \PV^i[\1_S].
%   \end{equation*}
%   Some equity holder strictly under-values the bond at the equilibrium price: from his perspective, the bond is overpriced and the firm prefers to issue no debt.
%   \item \emph{$\beta^\ast = \inf_{s \in S} f_s(k)$ (no-default constraint binding).} The left-derivative at $\beta = \beta^\ast$ is strictly positive, equivalent to
%   \begin{equation*}
%     q > \max_{i \in I^e(\beta^\ast)} \PV^i[\1_S].
%   \end{equation*}
%   \emph{Every} equity holder under-values the bond. The firm levers up to the maximal risk-free amount; in a model permitting risky debt, the firm would generally lever further at the cost of triggering default in adverse states.
% \end{itemize}
% The two cases reflect distinct demand-side situations: in the first, heterogeneous valuations among investors leave at least one equity holder unwilling to substitute towards the bond; in the second, the equity-bond wedge is uniform across all equity holders.
% \end{remark}

% \begin{remark}[Partial indeterminacy on a flat sub-interval]
% \label{rem:partial-MM}
% An economically natural intermediate case is the following: $V(k, \cdot)$ is strictly increasing on $[0, \beta_{\min}]$ and constant on $[\beta_{\min}, \inf_{s \in S} f_s(k)]$ for some $0 < \beta_{\min} < \inf_{s \in S} f_s(k)$. This belongs to the ($\neg$MM) regime of Proposition~\ref{prop:mm-dichotomy}---the global maximum is attained at $\inf_{s \in S} f_s(k)$---but the firm is indifferent across all $\beta \in [\beta_{\min}, \inf_{s \in S} f_s(k)]$, exhibiting \emph{partial} indeterminacy of capital structure. The numerical example in \citet[Figure~1]{BisinGottardiRuta2014} illustrates exactly this configuration, with $\beta_{\min} = 0.1828$ in their parameterization.
% \end{remark}


\subsection{The Modigliani--Miller dichotomy}
\label{subsec:mm-dichotomy}

The value map $V(k, \cdot)$ established in Lemma~\ref{lem:V-convex} is convex piecewise linear on the no-default interval. We henceforth write
\begin{equation*}
  \overline{\beta} \coloneqq \inf_{s \in S} f_s(k).
\end{equation*}
The convexity of $V(k, \cdot)$, combined with the elementary fact that a convex function attaining its maximum at an interior point of a closed interval must be constant on the entire interval, yields a sharp structural dichotomy.

\begin{proposition}[Modigliani--Miller dichotomy]\label{prop:mm-BGR}
Let $k \geq 0$ be fixed. Exactly one of the following holds.
\begin{itemize}
  \item[\textup{(MM)}] $V(k, \cdot)$ is constant on $[0, \overline{\beta}]$. Every $\beta \in [0, \overline{\beta}]$ is value-maximizing, and the firm's capital structure is indeterminate.
  \item[\textup{($\neg$MM)}] $V(k, \cdot)$ is not constant on $[0, \overline{\beta}]$. Every value-maximizing $\beta$ lies in $\{ 0, \overline{\beta} \}$.
\end{itemize}
\end{proposition}

\begin{proof}
By Lemma~\ref{lem:V-convex}, $V(k, \cdot)$ is convex piecewise linear, hence continuous, on $[0, \overline{\beta}]$. Suppose a value-maximizing $\beta^\ast \in (0, \overline{\beta})$ exists. We show that $V(k, \cdot)$ is constant on $[0, \overline{\beta}]$.

Fix $\beta \in [0, \overline{\beta}]$ with $\beta \neq \beta^\ast$. Because $\beta^\ast \in (0, \overline{\beta})$, the interval $[0, \overline{\beta}]$ contains a point $\gamma$ on the opposite side of $\beta^\ast$ from $\beta$. Pick $\lambda \in (0, 1)$ such that $\beta^\ast = \lambda \beta + (1 - \lambda) \gamma$. By convexity,
\begin{equation*}
  V(k, \beta^\ast) \leq \lambda V(k, \beta) + (1 - \lambda) V(k, \gamma).
\end{equation*}
Optimality of $\beta^\ast$ implies $V(k, \beta) \leq V(k, \beta^\ast)$ and $V(k, \gamma) \leq V(k, \beta^\ast)$, so
\begin{equation*}
  \lambda V(k, \beta) + (1 - \lambda) V(k, \gamma) \leq V(k, \beta^\ast).
\end{equation*}
Equality must hold throughout, forcing $V(k, \beta) = V(k, \beta^\ast)$. Since $\beta$ was arbitrary, $V(k, \cdot)$ is constant on $[0, \overline{\beta}]$, which is case~(MM).

If no value-maximizing $\beta^\ast$ lies in $(0, \overline{\beta})$, every value-maximizing $\beta$ lies in $\{0, \overline{\beta}\}$, which is case~($\neg$MM).
\end{proof}

\begin{remark}[Monotonicity of $V(k, \cdot)$ and the location of the optimum in the~($\neg$MM) regime]\label{rem:V-monotone}
By max-valuation pricing (Proposition~\ref{prop:max-pricing}), $\PV^i[\1_S] \leq q$ for every $i \in I$. Each function $g_i(\beta') \coloneqq \PV^i[f_1(k)] - \beta' \PV^i[\1_S]$ has slope $g_i'(\beta') = -\PV^i[\1_S] \in [-q, 0]$, so the max function $\widetilde{E}(k, \cdot) = \max_{i \in I} g_i(\cdot)$ has slope in $[-q, 0]$ wherever differentiable, hence the value map $V(k, \cdot) = \widetilde{E}(k, \cdot) + q \cdot - k$ has slope in $[0, q]$. In particular, $V(k, \cdot)$ is non-decreasing on $[0, \overline{\beta}]$.

It follows that in the~($\neg$MM) regime the unique maximizer that does not belong to a flat piece is the right endpoint $\overline{\beta}$. The corner $\beta = 0$ can be a maximizer in the~($\neg$MM) regime only as part of a flat piece on which $V(k, \cdot)$ already attains its global maximum, but then $V(k, \cdot)$ is constant on that flat piece, and the flat piece extends to $\overline{\beta}$ (by non-decrease combined with optimality of $0$), giving $V(k, \cdot)$ constant on $[0, \overline{\beta}]$ and contradicting the~($\neg$MM) hypothesis. Hence in~($\neg$MM), the firm pushes its debt to the no-default boundary: $\beta^\ast = \overline{\beta}$.
\end{remark}

\subsection{Equity holders are bond holders in the Modigliani--Miller regime}
\label{subsec:eh-are-bh}

The next proposition relates the value-function characterization in Proposition~\ref{prop:mm-BGR} to the marginal-holder sets $I^e$ and $I^b$ of Definition~\ref{def:marginal-holders}. The proof is direct: it uses only the constancy of $V(k, \cdot)$, the defining inequality of the Makowski maximum, and max-valuation pricing. No directional derivative is computed.

\begin{proposition}[Equity holders are bond holders, Modigliani--Miller regime]\label{prop:eh-are-bh}
Assume Assumption~\ref{ass:C} holds and let
\begin{equation*}
  \bigl\{ (k, \beta), (q, E), (c^i, \theta^i, \alpha^i)_{i \in I} \bigr\}
\end{equation*}
be a competitive equilibrium. If $V(k, \cdot)$ is constant on $[0, \overline{\beta}]$ \textup{(}case~\textup{(MM)} of Proposition~\textup{\ref{prop:mm-BGR}}\textup{)}, then $I^e(\beta') \subset I^b$ for every $\beta' \in [0, \overline{\beta})$.
\end{proposition}

\begin{proof}
Fix $\beta' \in [0, \overline{\beta})$ and let $i \in I^e(\beta')$. By the definition of the marginal-equity set,
\begin{equation*}
  \widetilde{E}(k, \beta') = \PV^i[f_1(k) - \beta' \1_S] = \PV^i[f_1(k)] - \beta' \PV^i[\1_S].
\end{equation*}
For any $\beta'' \in (\beta', \overline{\beta}]$, the Makowski maximum dominates the individual valuation:
\begin{equation}
  \widetilde{E}(k, \beta'') \geq \PV^i[f_1(k)] - \beta'' \PV^i[\1_S].
  \label{eq:makowski-dominance}
\end{equation}
Subtracting,
\begin{equation}
  ...
  \label{eq:diff-bound}
\end{equation}
Constancy of $V(k, \cdot)$ on $[0, \overline{\beta}]$ implies $\widetilde{E}(k, \beta'') + q \beta'' = \widetilde{E}(k, \beta') + q \beta'$, hence
\begin{equation}
  \widetilde{E}(k, \beta'') - \widetilde{E}(k, \beta') = q(\beta' - \beta'').
  \label{eq:diff-MM}
\end{equation}
Combining \eqref{eq:diff-bound} and \eqref{eq:diff-MM},
\begin{equation*}
  q(\beta' - \beta'') \geq (\beta' - \beta'') \PV^i[\1_S].
\end{equation*}
Since $\beta'' > \beta'$, $\beta' - \beta'' < 0$. Dividing by this negative quantity reverses the inequality:
\begin{equation*}
  q \leq \PV^i[\1_S].
\end{equation*}
Combined with max-valuation pricing $\PV^i[\1_S] \leq q$, this forces $\PV^i[\1_S] = q$, i.e., $i \in I^b$. Since $i \in I^e(\beta')$ was arbitrary, $I^e(\beta') \subset I^b$.
\end{proof}

\begin{remark}[The two regimes and their economic content]\label{rem:two-regimes}
The dichotomy in Proposition~\ref{prop:mm-BGR} carries direct economic meaning, sharpened by Remark~\ref{rem:V-monotone} and Proposition~\ref{prop:eh-are-bh}.
\begin{itemize}
  \item In the~\textup{(MM)} regime, the firm's capital structure is fully indeterminate. The market values debt and equity at consistent rates across every active margin: $\PV^i[\1_S] = q$ for every $i \in I^e(\beta)$ at every $\beta \in [0, \overline{\beta})$. There is no demand-side wedge that the firm can exploit by adjusting leverage.
  \item In the~\textup{($\neg$MM)} regime, the firm's capital structure is determinate. By monotonicity, the unique value-maximizing choice is $\overline{\beta}$. There exists at least one $\beta < \overline{\beta}$ where some equity holder strictly under-values the bond at the equilibrium price, i.e., $\PV^i[\1_S] < q$ for some $i \in I^e(\beta)$. \textup{(}On a possibly empty initial flat segment $[0, \beta^\ast]$ of $V(k, \cdot)$, the conclusion $I^e(\beta) \subset I^b$ may still hold; what is ruled out is the validity of this inclusion at \emph{every} $\beta < \overline{\beta}$, which would force $V(k, \cdot)$ to be constant on the entire interval and place us in the~\textup{(MM)} regime.\textup{)}
\end{itemize}
\end{remark}

\begin{remark}[Possible shapes of $V(k, \cdot)$ and the family of equilibria]\label{rem:V-shapes-and-BGR}
The combination of convexity (Lemma~\ref{lem:V-convex}) and monotonicity (Remark~\ref{rem:V-monotone}) restricts $V(k, \cdot)$ to one of only three slope profiles on $[0, \overline{\beta}]$, corresponding to the regimes of Proposition~\ref{prop:mm-BGR}:
\begin{itemize}
  \item Slope identically zero: $V(k, \cdot)$ is constant. This is the~\textup{(MM)} regime.
  \item Slope strictly positive everywhere: $V(k, \cdot)$ is strictly increasing on $[0, \overline{\beta}]$. This is the~\textup{($\neg$MM)} regime with unique optimum $\overline{\beta}$.
  \item Slope zero on $[0, \beta^\ast]$ and strictly positive on $[\beta^\ast, \overline{\beta}]$ for some $0 < \beta^\ast < \overline{\beta}$: $V(k, \cdot)$ has an initial flat segment followed by a strictly increasing one. This is the~\textup{($\neg$MM)} regime with unique optimum $\overline{\beta}$; the flat sub-interval $[0, \beta^\ast]$ does \emph{not} contain the firm's optimum.
\end{itemize}
The opposite shape, ``strictly increasing on $[0, \beta_{\min}]$ then constant on $[\beta_{\min}, \overline{\beta}]$'', is ruled out by convexity: it would require a concave kink at $\beta_{\min}$.
\end{remark}
