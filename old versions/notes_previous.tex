...
% [Abstract to be written once the book is complete.]
...
\begin{itemize}
  ...
  \item a Bernoulli utility function $u^i \vert \R_+ \to [-\infty, \infty)$;
...
\begin{enumerate}
  ...
  \begin{equation*}
    (c^i, \theta^i, \alpha^i) \in \argmax \bigl\{ U^i(c) \vert (c, \theta, \alpha) \in B^i(q, E) \bigr\};
  \end{equation*}
...
The firm belongs to its shareholders, i.e., to the set $\{ i \vert \xi^i > 0 \}$.
...
\begin{equation}\label{eq:feasible-set}
  \mathcal{D} \coloneqq \bigl\{ (y, \beta) \in Y \times \R^A \vert y_s \geq \kappa_s \cdot \beta, \ \forall s \in S \bigr\}.
\end{equation}
...
\begin{equation*}
  B^i(q, \lambda, y', \beta')
  \coloneqq
  \bigl\{ (c, \theta, \alpha) \in \R_+ \times \R_+^S \times \R^A \times \R_+ \vert \text{\eqref{eq:budget-0} and \eqref{eq:budget-s} hold with } E = \widetilde{E}(\lambda, y', \beta') \bigr\}.
\end{equation*}
...
\begin{equation}\label{eq:strong-unanimity}
  (y, \beta) \in \argmax \bigl\{ U^i(c') \vert (c', \theta', \alpha') \in B^i(q, \lambda, y', \beta'), \ (y', \beta') \in \mathcal{D} \bigr\}.
\end{equation}
\end{theorem}
...
\citet{BejanBidian2011} revisit the question of ownership structure and efficiency in large economies, and \citet{DierkerDierker2011} sharpen the welfare analysis of Drèze equilibria.
...
\begin{equation*}
  \mathcal{D}(j) \coloneqq \bigl\{ (y, \beta) \in Y(j) \times \R^A \vert y_s \geq \kappa_s \cdot \beta, \ \forall s \in S \bigr\},
\end{equation*}
...
\begin{enumerate}
  \item[(a)] for every $i \in I$,
  \begin{equation*}
    (c^i, \theta^i, \alpha^i) \in \argmax \bigl\{ U^i(c) \vert (c, \theta, \alpha) \in B^i(q, p) \bigr\};
  \end{equation*}
...
This chapter examines the firm's objective and the Modigliani--Miller theorem in an economy with short-sale constraints, following \citet{BisinGottardiRuta2014}.
...
\begin{equation}\label{eq:Y-BGR}
  Y \coloneqq \bigl\{ (y_0, (y_s)_{s \in S}) \in \R \times \R^S \vert y_0 \leq 0 \et y_s \leq f_s(-y_0), \ \forall s \in S \bigr\},
\end{equation}
...
\begin{equation}\label{eq:admissible-set}
  \mathcal{A} \coloneqq \bigl\{ (k, \beta) \in \R_+^2 \vert \inf_{s \in S} f_s(k) \geq \beta \bigr\}.
\end{equation}
...
\begin{enumerate}
  \item[(a)] for every $i \in I$,
  \begin{equation*}
    (c^i, \theta^i, \alpha^i) \in \argmax \bigl\{ U^i(c) \vert (c, \theta, \alpha) \in B^i(q, E, k, \beta) \bigr\};
  \end{equation*}
...
\begin{align}
  (u^i)'(c^i_0) &= \mu^i_0,
  \label{eq:foc-c0-BGR}\\
  \beta^i \nu^i_s (u^i)'(c^i_s) &= \mu^i_s, \quad \forall s \in S,
  \label{eq:foc-cs-BGR}\\
...
This is a strong epistemic hypothesis that the literature has addressed in various ways, e.g., by assuming perfect competition with a continuum of agents, or by embedding the model in a rational-expectations framework; see \citet{BisinGottardiRuta2014} for a discussion.
...
\begin{equation}\label{eq:strong-unanimity-BGR}
  U^i(c^i) = \max \bigl\{ U^i(c') \vert (c', \theta', \alpha') \in B^i(q, E', k', \beta'), \ (k', \beta') \in \mathcal{A} \bigr\}.
\end{equation}
...
\begin{enumerate}
  ...
  \begin{equation*}
    ...
    \et
    \sum_{i \in I} c^i_s \leq f_s(k) + \sum_{i \in I} \omega^i_s, \quad \forall s \in S;
  \end{equation*}
  ...
  \begin{equation*}
    c^i_s = \omega^i_s + \theta^i + \bigl[ f_s(k) - \beta \bigr] \alpha^i, \quad \forall s \in S.
  \end{equation*}
\end{enumerate}
\end{definition}
...
\begin{equation*}
  \widetilde{c}^i_s = \omega^i_s + \widetilde{\theta}^i + \bigl[ f_s(\widetilde{k}) - \widetilde{\beta} \bigr] \widetilde{\alpha}^i, \quad \forall s \in S.
\end{equation*}
...
A complete treatment is given by \citet{BisinGottardiRuta2014} and \citet{Makowski1980}.
\end{proof}
...
Let
\begin{align*}
  I^e &\coloneqq \bigl\{ i \in I \vert E = \PV^i\bigl[ f_1(k) - \beta \1_S \bigr] \bigr\}, \\
  I^b &\coloneqq \bigl\{ i \in I \vert q = \PV^i[\1_S] \bigr\}.
\end{align*}
...
A detailed development is provided by \citet{BisinGottardiRuta2014}.
...
\begin{equation}\label{eq:intermediary}
  \max \bigl\{ (q^+ - q^-) m - q \gamma \vert m \geq 0, \; \gamma \geq m \delta \bigr\}.
\end{equation}
...
The framework of this chapter is developed in \citet{BisinGottardiRuta2014}; an updated and refined version of this framework has subsequently been published as \citet{BisinClementiGottardi2025}.
...
\begin{equation*}
  x^i_0 = \omega^i_0 - q \cdot z^i, \qquad
  x^i_s = V_s(0, y^{-i}) \cdot z^i, \quad \forall s \in S.
\end{equation*}
...
\begin{equation}\label{eq:opt-effort}
  ...
  \coloneqq \argmax \bigl\{ u^i_1(x^i_1, e^i) \vert e^i \in E^i, \ x^i_1 = V(f^i(k^i, e^i), y^{-i}) z^i \bigr\}.
\end{equation}
...
At an equilibrium where the entrepreneur exerts high effort, $\overline{e}^i = e^i_h$, the equity payoff is $(y^i_h, y^i_h, y^i_\ell)$, and the marketed subspace is contained in $\{ w \in \R^3 \vert w_1 = w_2 \}$: every security written on the equity is constant across states $s_1$ and $s_2$.
...
\citet{BisinGottardiRuta2014} study the Makowski--style pricing and welfare properties of an economy with short-sale constraints and no bankruptcy, under full information.
...
\end{itemize}
The framework follows closely \citet{BisinGottardiRuta2014}, who extend their earlier analysis in this direction.
...
\begin{equation*}
  Y \coloneqq \bigl\{ (y_0, (y_s)_{s \in S}) \in \R \times \R^S \vert y_0 \leq 0 \et y_s \leq f_s(-y_0, e), \ \forall s \in S \bigr\}.
\end{equation*}
...
\begin{equation}\label{eq:manager-consumption}
  c^j_0 = \omega^j_0 + x_0,
  \qquad
  c^j_s = \omega^j_s + \delta_s(k, e, \beta) \alpha^m + b_s(k, e, \beta) \theta^m, \quad \forall s \in S.
\end{equation}
...
\begin{align}
  ...
  c^i_s
  &\leq \omega^i_s + \delta_s(k, e, \beta) \alpha^i + b_s(k, e, \beta) \theta^i, \quad \forall s \in S.
...
\begin{equation}\label{eq:opportunity-utility}
  U^i(q, E, k, \beta, e, W)
  \coloneqq \sup \bigl\{ U^i(c) \vert (c, \theta, \alpha) \in B^i(q, E, k, \beta, e, W) \bigr\}.
\end{equation}
...
\begin{enumerate}
  \item[(a)] for every $i \neq j$,
  \begin{equation*}
    (c^i, \theta^i, \alpha^i) \in \argmax \bigl\{ U^i(c) \vert (c, \theta, \alpha) \in B^i(q, E, k, \beta, e, W) \bigr\},
  \end{equation*}
...
\begin{equation}\label{eq:strong-unanimity-MH-CPP}
  U^i(c^i) = \max \bigl\{ U^i(c') \vert (c', \theta', \alpha') \in B^i(\widetilde{q}, \widetilde{E}, \widetilde{k}, \widetilde{\beta}, \widetilde{e}, \widetilde{W}), \ (\widetilde{j}, \widetilde{\phi}) \ \text{admissible} \bigr\},
\end{equation}
...
\begin{align*}
  ...
  c'_s
  &\leq \omega^i_s + \delta_s(\widetilde{k}, \widetilde{e}, \widetilde{\beta}) \alpha' + b_s(\widetilde{k}, \widetilde{e}, \widetilde{\beta}) \theta', \quad \forall s \in S.
\end{align*}
...
\begin{enumerate}
  ...
  \begin{equation*}
    k + \sum_{i \in I} c^i_0 \leq \omega_0
    \et
    \sum_{i \in I} c^i_s \leq f_s(k, e) + \sum_{i \in I} \omega^i_s, \quad \forall s \in S;
  \end{equation*}
  ...
  \begin{equation*}
    c^i_s = \omega^i_s + \delta_s(k, e, \beta) \alpha^i + b_s(k, e, \beta) \theta^i, \quad \forall s \in S;
  \end{equation*}
  \item[(iii)] \emph{Incentive compatibility}: there exists an agent $j \in I$ such that
  \begin{equation*}
    e \in \argmax \Bigl\{ u^j(c^j_0) + \beta^j \sum_{s \in S} \nu^j_s u^j(c^j_s(\widetilde{e})) - v^j(\widetilde{e}) \vert \widetilde{e} \in E \Bigr\},
  \end{equation*}
...
The framework of this chapter is developed by \citet{BisinGottardiRuta2014} as an extension of their earlier analysis surveyed in Chapter~\ref{ch:price-perceptions}.
...
\begin{enumerate}
  ...
  For every $i \in I$,
  \begin{equation*}
    (\bar{x}^i, \bar{z}^i) \in \argmax \bigl\{ U^i(x^i, \bar{Q}) \vert (x^i, z^i) \in B^i(\bar{q}, \bar{\tau}) \bigr\},
  \end{equation*}
...
\begin{equation}\label{eq:MQ08-SDF}
  \bar{\pi}(y) = \bar{m}(y) \bar{Q}(y), \qquad y \in Y.
\end{equation}
...
\begin{equation*}
  \bar{Q}(y) = \lambda^i \bar{\pi}(y), \qquad \forall y \in Y,
\end{equation*}
...
\begin{equation*}
  e^k \in \argmax \Bigl\{ \sum_{y \in Y} v^k(\tau^k(y)) Q(y, \widetilde{e}^k, \bar{e}^{-k}) - c^k(\widetilde{e}^k) \vert \widetilde{e}^k \geq 0 \Bigr\}.
  \tag{IC}\label{eq:IC}
\end{equation*}
...
\begin{enumerate}
  ...
  For every $i \in I$, $\bar{x}^i \in \argmax \{ U^i(x^i, \bar{Q}) \vert x^i \in B^i(\bar{\pi}, \bar{\tau}) \}$, with $\bar{Q} = Q(\cdot, \bar{e})$.
  \item[(b)] \emph{Firms maximize value.}
  For every firm $k \in K$, $(\bar{\tau}^k, \bar{e}^k) \in \argmax \{ \eqref{eq:firm-objective-MQ08} \vert (\tau^k, e^k) \in \mathcal{C}^k(\bar{e}^{-k}) \}$.
  \item[(c)] \emph{Markets clear.}
  \begin{equation*}
...
For the connection to the ``production approach'' of moral hazard, the reader is referred to \citet{MagillQuinzii2002} (Chapter~\ref{ch:moral-hazard} of these notes) and to \citet{BisinGottardiRuta2014} (Chapters~\ref{ch:price-perceptions} and~\ref{ch:moral-hazard-cpp}).
...
\begin{equation}\label{eq:S01-def}
  S_0(\beta) \coloneqq \{ s \in S \vert \beta(s) = 0 \}
  \et
  S_1(\beta) \coloneqq \{ s \in S \vert \beta(s) = 1 \},
\end{equation}
...
\begin{equation}\label{eq:F}
  \rho(s) + \gamma \beta(s) \leq f(s), \quad \forall s \in S.
  \tag{F}
\end{equation}
\end{definition}
...
\begin{equation*}
  s_0(\beta, \rho) \in \argmin \{ \rho(\sigma) \vert \sigma \in S_0(\beta) \}.
\end{equation*}
...
\begin{equation}\label{eq:HT-sharing}
  R_b + R_\ell = R, \qquad R_b \geq 0, \qquad R_\ell \geq 0.
\end{equation}
...
\begin{equation}\label{eq:HT-threshold}
  A \geq \overline{A}, \qquad \text{where} \qquad \overline{A} \coloneqq p_h \frac{B}{\Delta p} - (p_h R - I).
\end{equation}
\end{proposition}
...
\begin{equation}\label{eq:EM-I-kA}
  I = k A, \qquad \text{where} \qquad k \coloneqq \frac{1}{1 - p_h(R - B/\Delta p)} = \frac{1}{1 - \mathcal{P}}.
\end{equation}
...
\begin{equation}\label{eq:EM-v}
  \Pi_b^g = v A, \qquad v \coloneqq \frac{p_h B/\Delta p}{1 - p_h(R - B/\Delta p)} = \frac{p_h B/\Delta p}{p_h B/\Delta p - (p_h R - 1)}.
\end{equation}
...
\begin{equation}\label{eq:EM-rho-payoffs}
  \Pi_b^g = \frac{\rho_1 - \rho_0}{1 - \rho_0} A, \qquad \Pi_b = (\rho_1 - 1) I = (\rho_1 - 1) k A.
\end{equation}
...
\begin{equation}\label{eq:EM-decomposition}
  \text{payoff in success} = R^F I + R I, \qquad \text{payoff in failure} = R^F I.
\end{equation}
...
\begin{equation}\label{eq:EM-NPV-invariant}
  ...
  \qquad \text{i.e.,} \qquad \widehat{R}^F + (p_h + \tau) R = R^F + p_h R.
\end{equation}
...
\begin{equation}\label{eq:EM-pl-adjust}
  ...
  \qquad \widehat{\Delta p} = \widehat{p}_h - \widehat{p}_\ell = \Delta p.
\end{equation}
